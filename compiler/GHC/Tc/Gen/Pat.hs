...
tc_lmatchpat pat_ty@(Scaled _ ty) _ pat@(L l' (InvisTyVarPat x (L l name))) thing_inside
  = do { unExp <- readExpType ty
       ; let id = mkTyVar name unExp
       ; (res,_) <- tcCheckUsage name (scaledMult pat_ty) $ tcExtendTyVarEnv [id] thing_inside
