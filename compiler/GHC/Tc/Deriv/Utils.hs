...
hasStockDeriving clas
  ...
  where
    ...
    gen_list = [ (eqClassKey,          mk (simple_bindsM gen_Eq_binds)
                                          no_fam_insts)
               , (ordClassKey,         mk (simple_bindsM gen_Ord_binds)
                                          no_fam_insts)
               , (enumClassKey,        mk (simple_bindsM gen_Enum_binds)
                                          no_fam_insts)
               , (boundedClassKey,     mk (simple_binds gen_Bounded_binds)
                                          no_fam_insts)
               , (ixClassKey,          mk (simple_bindsM gen_Ix_binds)
                                          no_fam_insts)
               , (showClassKey,        mk (read_or_show_binds gen_Show_binds)
                                          no_fam_insts)
               , (readClassKey,        mk (read_or_show_binds gen_Read_binds)
                                          no_fam_insts)
               , (dataClassKey,        mk (simple_bindsM gen_Data_binds)
                                          no_fam_insts)
               , (functorClassKey,     mk (simple_binds gen_Functor_binds)
                                          no_fam_insts)
               , (foldableClassKey,    mk (simple_binds gen_Foldable_binds)
                                          no_fam_insts)
               , (traversableClassKey, mk (simple_binds gen_Traversable_binds)
                                          no_fam_insts)
               , (liftClassKey,        mk (simple_binds gen_Lift_binds)
                                          no_fam_insts)
               , (genClassKey,         mk (generic_binds (gen_Generic_binds Gen0))
                                          (generic_fam_inst (gen_Generic_fam_inst Gen0)))
               , (gen1ClassKey,        mk (generic_binds (gen_Generic_binds Gen1))
                                          (generic_fam_inst (gen_Generic_fam_inst Gen1)))
               ]
    ...
    generic_binds gen_fn loc dit
      = do { let tc = dit_rep_tc dit
           ; (binds, sigs) <- gen_fn loc dit
           ; let field_names = all_field_names tc
           ; return (binds, sigs, emptyBag, field_names) }

    generic_fam_inst gen_fn loc dit
      = do { let tc = dit_rep_tc dit
           ; fix_env <- getDataConFixityFun tc
           ; faminst <- gen_fn fix_env loc dit
           ; return [faminst] }
