...
mkUserTypeError :: ErrorItem -> TcReportMsg
...
mkGroupReporter :: (ReportErrCtxt -> [ErrorItem] -> TcM SolverReport)
...
reportGroup :: (ReportErrCtxt -> [ErrorItem] -> TcM SolverReport) -> Reporter
...
addDeferredBinding :: ReportErrCtxt -> SolverReport -> ErrorItem -> TcM ()
-- See Note [Deferring coercion errors to runtime]
...
mkIrredErr :: ReportErrCtxt -> [ErrorItem] -> TcM SolverReport
...
mkIPErr :: ReportErrCtxt -> [ErrorItem] -> TcM SolverReport
...
mkFRRErr :: ReportErrCtxt -> [ErrorItem] -> TcM SolverReport
mkFRRErr ctxt items
  = do { -- Zonking/tidying.
       ; origs <-
...
mkEqErr :: ReportErrCtxt -> [ErrorItem] -> TcM SolverReport
mkEqErr ctxt items
...
mkEqErr1 :: ReportErrCtxt -> ErrorItem -> TcM SolverReport
...
mkTyVarEqErr :: ReportErrCtxt -> ErrorItem
...
eqInfoMsgs :: TcType -> TcType -> [TcReportInfo]
...
mkBlockedEqErr :: ErrorItem -> TcReportMsg
...
--------------------
mkMismatchMsg :: ErrorItem -> Type -> Type -> TcReportMsg
mkMismatchMsg item ty1 ty2 =
  case orig of
...
mkDictErr :: HasDebugCallStack => ReportErrCtxt -> [ErrorItem] -> TcM SolverReport
...
mk_dict_err ctxt (item, (matches, unifiers, unsafe_overlapped))
  ...
  where
    ...
    cannot_resolve_msg :: ErrorItem -> [ClsInst] -> RelevantBindings
                       -> [ImportError] -> [GhcHint] -> TcReportMsg
...
relevantBindings :: Bool  -- True <=> filter by tyvar; False <=> no filtering
                 ...
                 -> TcM (ReportErrCtxt, RelevantBindings, ErrorItem)
