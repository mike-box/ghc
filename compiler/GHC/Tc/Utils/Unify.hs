...
zipLMatchPats fun_ty matchpats = zip_matchpats [] emptyTCvSubst fun_ty matchpats
  where
    ...
    zip_matchpats acc subst ty matchpats
      ...
      = case (isNamedBinder tb', pat) of
          (True, pat'@(L _ (InvisTyVarPat _ _))) -> zip_matchpats ((tb', Just pat') : acc) subst' ty' pats
          (True, pat'@(L _ (InvisWildTyPat _ ))) -> zip_matchpats ((tb', Just pat') : acc) subst' ty' pats
          _ -> case (isAnonTyCoBinder tb', pat) of
                 (True, pat'@(L _ (VisPat _ _))) -> zip_matchpats ((tb', Just pat') : acc) subst' ty' pats
...
matchExpectedFunTys :: forall a.
                    ...
                    -> [LMatchPat GhcRn]
                    -> ExpSigmaType
                    -> ([ExpTyCoBinder] -> ExpSigmaType -> TcM a)
                    -> TcM (HsWrapper, a)
...
matchExpectedFunTys herald ctx lmatchpats orig_ty thing_inside
  ...
  where
    ...
    go bndrs lmatchpats ty
      | (tvs, theta, _) <- tcSplitSigmaTy ty
      , not (null tvs && null theta)
      , all isVis lmatchpats
      ...
      = do { traceTc "zipped binders are" (ppr zipped_binders)
           ; case (pair, ty) of
               ((Anon VisArg scaled_arg_ty, Just pat@(L _ (VisPat _ _))), FunTy { ft_af = VisArg, ft_res = res_ty }) -> do
                 { (wrap_res, result) <- go (toExpBinder bndr : bndrs) pats res_ty
               ...
               ((Named (Bndr _ Specified), Just pat@(L _ (InvisTyVarPat _ _))), ForAllTy (Bndr var _) ty') -> do
                 { (wrap_res, result) <- go (toExpBinder bndr : bndrs) pats ty'
               ...
               ((Named (Bndr _ Specified), Just (L _ (InvisWildTyPat _))), ForAllTy (Bndr var _) ty') -> do
                 ...
                 ; let wrap_gen = WpTyLam var
                 ; return (wrap_gen <.> wrap_res, result) }
               _ -> go (toExpBinder bndr : bndrs) pats rest_ty
            }

    go bndrs lmatchpats ty@(TyVarTy tv)
      | isMetaTyVar tv
      = do { cts <- readMetaTyVar tv
           ; case cts of
               Indirect ty' -> go bndrs lmatchpats ty'
               Flexi        -> defer bndrs lmatchpats (mkCheckExpType ty) }
    ...
    defer :: [ExpTyCoBinder] -> [LMatchPat GhcRn] -> ExpSigmaType -> TcM (HsWrapper, a)
    defer bndrs pats fun_ty
      = do { bndrs'        <- mapM patToExpBinder pats
    ...
    mk_ctxt bndrs res_ty env
      ...
      where
        bndrs' = map (checkingExpBinder "matchExpectedFunTys") (reverse bndrs)
...
mkPiTysMsg env herald bndrs res_ty lmatchpats
  = do { let fun = mkPiTys bndrs res_ty
       ...
       ; let (all_arg_tys, _) = splitFunTys fun_ty
             ...
             forall_herald = herald <+> speakNOf all_invis_pats (text "invisible pattern")
             ...
             msg | all_invis_pats <= n_forall_args && all_lpats <= n_fun_args  -- Enough args, in the end
                 ...
                 = hang (forall_herald <> comma)
                      2 (sep [ text "but its type" <+> quotes (pprType fun_ty)
                             , if n_forall_args == 0 then text "has none"
                               else text "has only" <+> speakN n_forall_args])
                 | otherwise
