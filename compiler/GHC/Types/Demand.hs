...
data SubDemand
  ...
  -- For 'Call', it's a bit more complicated: Only @Poly b C_0N@ is fully
  -- semantically equivalent to @Call C_0N (Poly Boxed C_0N)@. For other
  -- cardinalities @n@, we only have @Poly b n <= Call n (Poly Boxed n)@,
  -- because otherwise 'plusSubDmd' won't be well-defined (#21085).
  -- 'viewCall' and 'viewProd' do the expansion of 'Poly's, 'mkProd' and
  -- 'mkCall' the contraction to 'Poly's (if permitted).
  --
...
-- mkCall C_1N sd@(Poly Boxed C_1N) = sd -- DON'T do this. It makes plusSubDmd undefined! #21085
...
multSubDmd C_11 sd           = sd -- An optimisation
...
-- The following two lines compute the same result as the last line, but won't
...
plusDmd (n1 :* sd1) (n2 :* sd2) = case (isStrict n1, isStrict n2) of
  ...
  (False, True)  -> plusCard n1 n2 :* plusSubDmd (lazify sd1) sd1
...
strictifyDmd (n :* sd) = strictifyCard n :* sd
